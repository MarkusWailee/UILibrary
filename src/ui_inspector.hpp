...
namespace UI::ToolKit
{
    ...
    void Inspector_recurse(Internal::TreeNode* node, int depth)
    {
        ...
        line.background_color = {70, 60, 60, 255};
        const Internal::Box& box = node->box;

        const char* text = box.text? box.text: "Box";

        BeginBox(element_drop_down);
        for(int i = 0; i<=depth; i++)
        {
            ...
            InsertText(StringFormat("[S:16]+ %s Line: %d", text, box.debug_line), true);
        EndBox();
